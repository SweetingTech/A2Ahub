...
function settings(args) {
  ...
  const roomId = option("room"),
    name = option("name", harness === "codex" ? "Codex" : "Claude");
  if (!["codex", "claude-code"].includes(harness))
    throw new Error("Supported existing-session adapters: codex, claude-code.");
  if (!uuid(sessionId) || !uuid(roomId))
    throw new Error(
      "An exact existing session UUID and Hub room UUID are required.",
    );
  if (
    harness === "codex" &&
  ...
  const dir = path.join(
    ...
    "sessions",
    createHash("sha256")
      .update(`${origin.origin}/${name}/${harness}/${sessionId}/${roomId}`)
      .digest("hex"),
  );
...
export async function serve(config, { signal } = {}) {
  ...
  try {
    await serveLocked(config, signal);
  } finally {
    fs.rmSync(lockFile, { force: true });
  }
}

async function serveLocked(config, signal) {
  ...
  if (config.harness === "claude-code") {
    const { channelTransport } = await import("./adapters/claude-channel.mjs");
  ...
  const monitor = setInterval(async () => {
    if (config.harness !== "codex" || checkingHost) return;
    checkingHost = true;
    try {
      const host = await codexHost(config.hostPid);
...
async function main() {
  ...
  if (!(
    (config.command === "attach" && config.harness === "codex") ||
    (config.command === "channel" && config.harness === "claude-code")
  ))
    throw new Error("Use attach for Codex, or channel for Claude Code.");
  if (config.harness === "codex") {
    const host = await codexHost(config.hostPid);
